...
// Add event listener for the file input
document.getElementById("importFile").addEventListener("change", importFromJsonFile);
// Mock server URL (replace this with the actual server URL in a real application)
const serverUrl = "https://jsonplaceholder.typicode.com/posts"; // Using posts to simulate quotes
...
async function fetchQuotesFromServer() {
  try {
    ...
    const serverQuotes = serverData.map(post => ({
      text: post.title, 
      category: post.body.substring(0, 20) // For demonstration, using a snippet of the body as the category
    }));
    return serverQuotes;
  } catch (error) {
    console.error("Error fetching server data:", error);
...
document.addEventListener("DOMContentLoaded", function() {
  createAddQuoteForm();
  showRandomQuote(); // Display a random quote on load
  setInterval(syncQuotes, 5000); // Sync every 5 seconds (for simulation purposes)
});
...
async function syncQuotes() {
  ...
  // Display a message in the UI indicating that quotes are synced
...
// Function to display sync message on UI
